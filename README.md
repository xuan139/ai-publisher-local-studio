...
- 專案列表頁提供全屏 `新建電子書向導`
- 支援直接貼上整本內容後自動拆章
- 支援手動新增自訂章節，例如 `西游记第一章 / 第二章节 / 第一回`
- 自動拆章拆段
- 導入後自動保存完整整書 `source_book.txt`
...
- 文本準備頁已整理成更清爽的編輯工作台：
  - 緊湊動作列與彈窗式匯入
  - `寫稿 / 批注 / 詞典 / 版本` 四個工作分頁
  - 段落批注、修訂記錄、術語表 / 發音詞典、章節版本 diff
- 主界面左側主選單可收起，方便在文本準備頁騰出更寬的編輯空間
- 介面語言支援 `繁體中文 / 简体中文 / English / 日本語 / 한국어 / ไทย` 切換，預設為繁體中文
- 全屏 `新建電子書向導` 已支援多語切換
- `經營管理` 已提供：
  - 多幣種與匯率
  - 權利與合同
...
- 有聲書：
  `登入 -> 專案管理 -> 全屏電子書向導 / 文本匯入 -> 拆章拆段 -> 人物識別 / 角色綁定 -> 編輯朗讀稿 -> 配置聲線 -> 生成音訊 -> 審核 -> 章節渲染 -> ZIP 匯出`
- 漫畫：
  `登入 -> 專案管理 -> 漫畫腳本 -> 分鏡工作台 -> 畫格生成 -> 頁面排版 -> 審核 -> 導出`
- 經營管理：
  `登入 -> 選取專案 -> 經營管理 -> 設定基準幣 / 匯率 -> 維護權利 / 渠道 / 廣告合作 / 成本 / 銷售 / 版稅 -> 匯出經營報表 ZIP`
...
- 預設為 `繁體中文`
- 可在登入頁右上角或登入後右上角切換為 `简体中文 / English / 日本語 / 한국어 / ไทย`

### 電子書快速導入

建立專案後，可直接在 `專案列表` 頁使用新的全屏向導：

1. 選中專案後，點擊 `新建電子書` 或 `開啟全屏向導`
2. 方式 A：直接貼上整本內容，按 `貼上並拆章`
3. 方式 B：逐章輸入章節標題與正文，按 `建立章節`
4. 導入完成後，系統會同步保存 `source_book.txt`，並可直接接到 `Generate / Review / Export`
5. 如果你先切到 `English / 日本語 / 한국어 / ไทย`，全屏向導也會同步顯示對應語言

### 經營管理快速入口

建立或選取專案後，可直接在左側進入 `經營管理`：

1. 在 `多幣種與匯率` 設定專案基準幣與換算匯率
2. 在 `廣告合作專區` 維護廣告商、合作檔期、簽約額與已回款
3. 在 `發行渠道 / 成本核算 / 銷售回傳 / 版稅與分成` 持續補齊經營台帳
4. 於 `經營報表` 一鍵匯出 ZIP，內含 `HTML / JSON / CSV`
5. 如果你切換介面語言，`經營管理` 頁的標題、表單、狀態與提示也會同步切換

### 專案封面與書介

目前 `專案列表` 與右側 `專案摘要` 已可直接顯示：

- 專案封面
- 專案簡介 / 書介文案

這讓本機版不只適合製作流程，也更適合拿來做演示型電子書樣書。

### 編輯工作台補充

進入 `文本準備` 後，現在可直接使用四個編輯分頁：

- `寫稿`：調整 `tts_text`、角色、聲線與修訂說明
- `批注`：逐段記錄問題、修改建議與處理狀態
- `詞典`：維護專有名詞、推薦寫法與發音詞典
- `版本`：保存章節快照並比對歷史版本

如果文本頁面空間不足，也可直接收起左側主選單，讓編輯區更寬。

### 演示專案建議

若你要做現場演示或內部展示，可優先建立這幾類樣書：
