...
`登入 -> 專案管理 -> 全屏電子書向導 / 匯入文本 -> 拆章拆段 -> 編輯朗讀稿 -> 配置聲線 -> 生成音訊 -> 審核 -> 章節渲染 -> ZIP 匯出`
...
1. 建立一個新專案
   如果只是做演示，也可以直接用經典名著試讀方向建立，例如：
   - `西游记白话版`
   - `蜀山奇侠`
   - `红楼梦`
   - `水浒传`
   - `三国演义`
2. 回到 `專案列表` 頁，點 `新建電子書` 或 `開啟全屏向導`
3. 用以下任一方式導入內容：
   - 貼上整本內容，點 `貼上並拆章`
   - 手動新增單章，填標題與正文後點 `建立章節`
   - 或在 `Text Prep` 傳統上傳 `.txt / .md / .docx / .epub`
   - 若已切換語言，全屏向導與右側章節地圖也會跟著切換
4. 確認章節與段落已生成，且專案右側可看到整書 TXT 下載入口
5. 在 `Text Prep` 確認上方動作列可看到：
   - `選擇檔案`
   - `從路徑匯入`
   - `貼上並拆章`
   - `手動新增章節`
...
6. 在文本準備頁確認可切換：
   ...
   - `詞典`
   - `版本`
7. 保存一個段落，並嘗試新增一條批注或一條術語詞典
8. 如專案已補封面與簡介，確認 `專案列表` 與右側 `專案摘要` 可正常顯示
9. 在生成工作台生成一段音訊
10. 到審核頁確認可播放音訊
11. 到匯出頁確認 ZIP 匯出與整書 TXT 下載都可用
12. 可切到 `經營管理` 驗證多語是否也正常套用到表單與摘要

若要驗證擴展設定是否可用，也可額外檢查：

13. 進入「系統設定」頁
14. 確認目前專案可看到 `漫畫設定 / Video 設定`
15. 儲存一組模型配置並重新進入頁面確認有回顯
